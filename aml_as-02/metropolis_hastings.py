...
def sample_x(x_proposed, x_initial, proportional_function, *args):
    top, additional_results_prop = proportional_function(x_proposed, args)
    bottom, _ = proportional_function(x_initial, args)
    acceptance_ratio = top / bottom
    acceptance_prob = min(1, acceptance_ratio)
    u = np.random.uniform(0, 1, size=1)
    if u <= acceptance_prob:
        return x_proposed, 1, [top, additional_results_prop]
    else:
        return x_initial, 0, [top, additional_results_prop]
